...
void fast_preview() {
	...
	// 调用读取
	LFIO::LoadLookUpTables("../../data/calibration/slice_1.bin", slice_maps, winSize);
	...
	timer.start();
	cv::remap(demosaic_img, sliced_img, slice_x, slice_y, cv::INTER_CUBIC, cv::BORDER_REPLICATE);
	timer.stop();
	timer.print_elapsed_ms();
	...
	LFIO::LoadLookUpTables("../../data/calibration/lut_dehex.bin", dehex_maps, winSize);
	...
	cv::Mat dehex_img;
	timer.start();
	cv::remap(sliced_img, dehex_img, dehex_x, dehex_y, cv::INTER_CUBIC, cv::BORDER_REPLICATE);
	timer.stop();
	timer.print_elapsed_ms();
	...
	cv::remap(slice_x, fused_x, dehex_x, dehex_y, cv::INTER_LINEAR, cv::BORDER_REPLICATE);
	cv::remap(slice_y, fused_y, dehex_x, dehex_y, cv::INTER_LINEAR, cv::BORDER_REPLICATE);
	cv::Mat result;
	cv::remap(demosaic_img, result, fused_x, fused_y, cv::INTER_LINEAR, cv::BORDER_REPLICATE);
...
void get_sai() {
	...
	LFIO::LoadLookUpTables("../../data/calibration/lut_extract_9.bin", isp.maps.extract, slice_size);
	LFIO::LoadLookUpTables("../../data/calibration/lut_dehex.bin", isp.maps.dehex, dehex_size);
