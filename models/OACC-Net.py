...
class Net(nn.Module):
    ...
    def forward(self, x, dispGT=None):
        # 替换 einops.rearrange
        # 原代码: lf = rearrange(x, 'b c (a1 h) (a2 w) -> b c a1 a2 h w', a1=self.angRes, a2=self.angRes)
        b, c, h_tot, w_tot = x.shape
        h_sub = h_tot // self.angRes
        w_sub = w_tot // self.angRes
        
        # 步骤1: 拆分高和宽 [b, c, a1, h, a2, w]
        x_reshaped = x.view(b, c, self.angRes, h_sub, self.angRes, w_sub)
        
        # 步骤2: 调整维度顺序到 [b, c, a1, a2, h, w] -> 对应 lf
        lf = x_reshaped.permute(0, 1, 2, 4, 3, 5)

        # 原代码: x = rearrange(x, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        # 步骤3: 将 a1 和 a2 合并 [b, c, a1*a2, h, w] -> 对应 x
        x = lf.reshape(b, c, self.angRes * self.angRes, h_sub, w_sub)
...
class BuildCost(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        x = x.permute(0, 2, 1, 3, 4).reshape(b * aa, c, h, w)
        bdr = (self.angRes // 2) * self.maxdisp
        pad = nn.ZeroPad2d((bdr, bdr, bdr, bdr))
        x_pad = pad(x)
        
        # ================== 修改重点 ==================
        # 原代码 (会导致 h_pad 是 Tensor 类型):
        # _, _, h_pad, w_pad = x_pad.shape
        
        # 新代码 (强制转换为 int，打断 Trace):
        # 既然我们现在是导出固定尺寸模型，x_pad 的形状是确定的。
        # 使用 int() 强转，告诉编译器“这就是个常数，别把它当变量”。
        h_pad = int(x_pad.shape[2])
        w_pad = int(x_pad.shape[3])
        # =============================================

        # 之前的代码改动（reshape/permute）保持不变
        x_pad = x_pad.view(b, self.angRes, self.angRes, c, h_pad, w_pad)
        x_pad = x_pad.permute(0, 3, 1, 4, 2, 5)
        x_pad = x_pad.reshape(b, c, self.angRes * h_pad, self.angRes * w_pad)

        mask_avg = torch.mean(mask, dim=1)
        cost = []
        for d in range(self.mindisp, self.maxdisp + 1):
            # 这里的 h_pad 和 w_pad 现在是 int 类型
            # 这里的 d 也是 int 类型
            # 所以 dila 会变成 [160, 160] 这样的纯数字列表，满足 ONNX 要求
...
class CALayer(nn.Module):
    ...
    def forward(self, x):
        # 使用 mean 替代 AdaptiveAvgPool3d，保留深度维(dim=2)，压平 H 和 W (dim=3,4)
        # x 形状: [B, C, D, H, W] -> y 形状: [B, C, D, 1, 1]
        y = x.mean(dim=(3, 4), keepdim=True) 
        y = self.conv_du(y)
        return x * y
...
class ModulateConv2d(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # 【修改方案】直接使用 reshape 替代 nn.Fold(kernel_size=1)
        # x_unfold_modulated 的形状是 [B, Channels, L]
        # 我们需要将其恢复为 [B, Channels, H, W]
        # H 和 W 就是 mask 的最后两个维度
        H, W = mask.shape[2], mask.shape[3]
        x_modulated = x_unfold_modulated.view(x_unfold_modulated.shape[0], x_unfold_modulated.shape[1], H, W)
...
if __name__ == '__main__':
    ANG_RES = 9  
    MODEL_PATH = "models/OACC-Net.pth.tar" 
    ONNX_PATH = f"models/OACC-Net_{ANG_RES}x{ANG_RES}.onnx"
