...
struct ISPConfig {
	BayerPattern pattern = BayerPattern::GRBG;
	int bit = 10;
...
public:
	...
	LFIsp &preview(float exposure = 1.5f) {
		...
		int code = get_demosaic_code(config_.pattern);
	...
	LFIsp &dpc_fast(int threshold = 100) {
		...
		if constexpr (std::is_same_v<T, uint16_t>) {
			// 针对 16-bit 通道 (u16)：
			return dpc_simd_u16(lfp_img_, threshold << (config_.bit - 10));
		} else if constexpr (std::is_same_v<T, uint8_t>) {
			// 针对 8-bit 通道 (u8)：
	...
	LFIsp &resample() {
		int num_views = maps.slice.size() / 2;
...
#pragma omp parallel for schedule(dynamic)
		for (int i = 0; i < num_views; ++i) {
			cv::Mat temp;
			cv::remap(src, temp, maps.slice[i * 2], maps.slice[i * 2 + 1],
					  cv::INTER_LINEAR, cv::BORDER_REPLICATE);
			cv::remap(temp, sais[i], maps.dehex[0], maps.dehex[1],
					  cv::INTER_LINEAR, cv::BORDER_REPLICATE);
		}
...
#pragma omp parallel for
	...
	LFIsp &raw_to_8bit_with_gains_simd_u8(cv::Mat &dst_8u,
		...
		int bl_shift = (config_.bit > 8) ? (config_.bit - 8) : 0;
...
#pragma omp parallel for
	...
	int get_demosaic_code(BayerPattern pattern) const {
		// OpenCV 的 Bayer 转换码命名规则是 BayerXX2BGR
		// 其中 XX 是这一行的前两个像素颜色
		switch (pattern) {
			case BayerPattern::GRBG:
				return cv::COLOR_BayerGR2RGB; // 第0行是 G, R
			case BayerPattern::RGGB:
				return cv::COLOR_BayerRG2RGB; // 第0行是 R, G
			case BayerPattern::GBRG:
				return cv::COLOR_BayerGB2RGB; // 第0行是 G, B
			case BayerPattern::BGGR:
				return cv::COLOR_BayerBG2RGB; // 第0行是 B, G
			default:
				// 默认或者抛出异常
